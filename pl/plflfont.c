...
void
pl_fill_in_ft_font(gs_font_base *pfont, TT_Face face)
{	gs_make_identity(&pfont->FontMatrix);
	...
	/* This is probably not a good way to use the face handle */
        uid_set_UniqueID(&pfont->UID, (unsigned long)face.z);
...
int
pl_load_ft_font(TT_Face face, gs_font_dir *pdir, gs_memory_t *mem,
...
#define style_word(posture, width, structure) \
	...
	{"Courier New", {'C','o','u','r','i','e','r',' ',' ',' ',' ',' ',' ',' ',' ',' '},
	 {0, 0, fp_pitch_value_cp(60), 0, style_word(0, 0, 0), 0, face_val(3, agfa)},
	 cc_alphabetic},
	...
	{"Courier New Bold", {'C','o','u','r','i','e','r',' ',' ',' ',' ',' ',' ',' ','B','d'},
	 {0, 0, fp_pitch_value_cp(60), 0, style_word(0, 0, 0), 3, face_val(3, agfa)}, 
	 cc_alphabetic },
	{"Courier New Italic", {'C','o','u','r','i','e','r',' ',' ',' ',' ',' ',' ',' ','I','t'},
	 {0, 0, fp_pitch_value_cp(60), 0, style_word(1, 0, 0), 0, face_val(3, agfa)},
	 cc_alphabetic },
	{"Courier New Bold Italic", {'C','o','u','r','i','e','r',' ',' ',' ',' ',' ','B','d','I','t'},
	 {0, 0, fp_pitch_value_cp(60), 0, style_word(1, 0, 0), 3, face_val(3, agfa)},
	 cc_alphabetic},
	{"Letter Gothic", {'L','e','t','t','e','r','G','o','t','h','i','c',' ',' ',' ',' '},
	 {0, 0, fp_pitch_value_cp(50), 0, style_word(0, 0, 0), 0, face_val(6, agfa)},
	 cc_alphabetic},
	{"Letter Gothic Bold",
	 {'L','e','t','t','e','r','G','o','t','h','i','c',' ',' ','B','d'},
	 {0, 0, fp_pitch_value_cp(50), 0, style_word(0, 0, 0), 3, face_val(6, agfa)},
	 cc_alphabetic},
	{"Letter Gothic Italic", 
	 {'L','e','t','t','e','r','G','o','t','h','i','c',' ',' ','I','t'},
	 {0, 0, fp_pitch_value_cp(50), 0, style_word(1, 0, 0), 0, face_val(6, agfa)},
	 cc_alphabetic},
...
#undef face_val
    ...
    if ( error ) {
        dprintf( "could not create engine instance\n");
    }
	...
	for ( tmp_pathp = tmp_path_copy;
		...
		error = TT_Open_Face( engine, tmp_path_copy, &face );
		if ( error ) {
		...
		{
		    TT_String *ptt_buffer;
		    TT_UShort tt_buffer_length;
		    byte *bptr = buffer;
		    error = TT_Get_Name_String( face, 4, &ptt_buffer, &tt_buffer_length );
		    if ( error ) {
		    ...
		    while( tt_buffer_length-- )
			if ( isprint( *ptt_buffer )  )
			    *bptr++ = *ptt_buffer++;
			else
			    ptt_buffer++;
		    /* null terminate */
