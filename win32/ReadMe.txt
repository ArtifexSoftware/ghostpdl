...
There is currently a bug in the makefiles that means that some of the
different targets here share an autogenerated file. On some computers
(possibly only multi core/processor ones running VS2008 or later)
building will result in each target being made in parallel. If you are
unlucky with timing this will result in different parallel builds attempting
to use the same file at the same time, and give build errors.

A proper solution requires a fix to the Makefile. A workaround is just to
retry the make, or (better) to make each target in turn.

This issue is being tracked at bugs.ghostscript.com as bug 691061.
