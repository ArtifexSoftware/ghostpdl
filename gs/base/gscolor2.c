...
{
    int             code = 0;
    gs_color_space  *cs_old = pgs->color[pgs->current_color].color_space;
    gs_client_color cc_old = *pgs->color[pgs->current_color].ccolor;
    ...
    if (pcs->id != cs_old->id) {
        ...
        if ( (code = pcs->type->install_cspace(pcs, pgs)) < 0          ||
            ...
            pgs->color[pgs->current_color].color_space = cs_old;
    ...
    if (code >= 0) {
	pgs->color[pgs->current_color].color_space->pclient_color_space_data =
...
{
    return pgs->color[pgs->current_color].color_space;
}
...
{
    gs_color_space *    pcs = pgs->color[pgs->current_color].color_space;
    gs_client_color     cc_old = *pgs->color[pgs->current_color].ccolor;
    ...
    *pgs->color[pgs->current_color].ccolor = *pcc;
    (*pcs->type->restrict_color)(pgs->color[pgs->current_color].ccolor, pcs);
...
{
    return pgs->color[pgs->current_color].ccolor;
}
...
{
    return pgs->color[pgs->current_color].dev_color;
}
