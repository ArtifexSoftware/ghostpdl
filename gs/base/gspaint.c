...
/* Fill the current path using a specified rule. */
static int
fillstroke_with_rule(gs_state * pgs, int rule)
{
    int code;
...
/* Fill using the winding number rule */
int
gs_fillstroke(gs_state * pgs)
{
    pgs->device->sgr.stroke_stored = false;
    return fillstroke_with_rule(pgs, gx_rule_winding_number);
}
/* Fill using the even/odd rule */
int
gs_eofillstroke(gs_state * pgs)
{
    pgs->device->sgr.stroke_stored = false;
    return fillstroke_with_rule(pgs, gx_rule_even_odd);
}
