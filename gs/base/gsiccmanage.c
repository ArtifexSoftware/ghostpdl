    ...
    uint *                          psize )
{
    ...
    /* Just write out the default profile code if profile is
       one of our default color types */

    if (icc_profile->default_match != DEFAULT_NONE && offset == 0 && 
        left == sizeof(icc_profile->default_match)) {

	gsicc_profile_t default_type = icc_profile->default_match; /* Ensure sizeof(default_type). */

	memcpy(data, &(icc_profile->default_match), sizeof( default_type ));
	*psize = sizeof(default_type);

        return 0; 

    }

    /* Just write out the hash code.  Profile must be in the profile cache */

    if (offset == 0 && left == sizeof( icc_profile->hashcode )) {

	/* A known icc profile. 
	   Just write the hash code. */

	int64_t hash = icc_profile->hashcode; /* Ensure sizeof(hash). */

	memcpy(data, &(icc_profile->hashcode), sizeof( hash ));
	*psize = sizeof(hash);
    ...
    size_h = sizeof( gsicc_serialized_profile_t );
    size = icc_profile->buffer_size + size_h;

    /* If data was sent with NULL the clist writer just wanted the size. */ 
    if (data == NULL) {
	*psize = size;
	return 0;
    }

    if (offset == 0) {	/* Take care of the non buffer portions of the profile */
	...
	data += sizeof(profile_data);
	offset += sizeof(profile_data);

    }

    /* Now take care of the buffer, which is the raw icc profile */
    ...
    if (offset <= size) {
        ...
        ptr = icc_profile->buffer;
        memcpy(data, ptr + (offset - size_h), u);
