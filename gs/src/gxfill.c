		 ...
		 const gx_device_color * pdevc, const gx_clip_path * pcpath)
{
	...
	*/
	...
	code = gx_cpath_intersect(&cpath_intersection, ppath, params->rule, (gs_imager_state *)pis);
	/* Do fill : */
	if (code >= 0) {
		...
		code = pdevc->type->fill_rectangle(pdevc,
			    clip_box.p.x, clip_box.p.y, 
			    clip_box.q.x - clip_box.p.x, 
			    clip_box.q.y - clip_box.p.y,
			    dev, pis->log_op, rs);
	    } else {
