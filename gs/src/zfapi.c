...
{   if (rc == 0)
	return 0;
    eprintf2(imemory, "Error: Font Renderer Plugin ( %s ) return code = %d\n", I->ig.d->subtype, rc);
...
{   os_ptr op = osp;
    ...
    int code = font_param(imemory, op - 1, &pfont), code1;
    ...
    if (pfont->FontType == ft_CID_encrypted && v == NULL) {
        if ((code = build_proc_name_refs(imemory, &build, 
					 ".FAPIBuildGlyph9", ".FAPIBuildGlyph9")) < 0)
	    return code;
    } else
        if ((code = build_proc_name_refs(imemory, &build, 
					 ".FAPIBuildChar", ".FAPIBuildGlyph")) < 0)
	    return code;
...
{   if ((rast->line_step & (align_bitmap_mod - 1)) == 0)
        ...
        if (p == NULL)
            return_error(dev1->memory, e_VMerror);
...
retry_oversampling:
    ...
    if (bCID) {
	int_param(imemory, op, 0xFFFF, &client_char_code);
        make_null(&char_name);
    } else if (r_has_type(op, t_integer)) {
	/* Translate from PS encoding to char name : */
	ref *Encoding;
	int_param(imemory, op, 0xFF, &client_char_code);
	    ...
	    if (array_get(imemory, Encoding, client_char_code, &char_name) < 0)
	        if ((code = name_ref(imemory, (const byte *)".notdef", 
				     7, &char_name, -1)) < 0)
		    return code;
	} else
            return_error(imemory, e_invalidfont);
	...
	ref *CharStrings, *glyph_index;
        ...
        */
	    ...
	    ref *Decoding, *char_code;
		    ...
		    int_param(imemory, char_code, 0xFFFF, &cr.char_code);
	    }
        }
    } 
	...
	if (!can_replace_metrics) {
	    ...
	    char_bbox.p.x = 0;
	    gs_distance_transform(imemory, (metrics.bbox_x0 / em_scale_x - sbw[0]), 
...
{   /* Stack : <font> <code|name> --> - */
    ...
    gs_font *pfont;
    int code = font_param(imemory, osp - 1, &pfont);
...
{   /*  The alghorithm is taken from %Type9BuildGlyph - see gs_cidfn.ps .  */
    ...
    check_type(imemory, op[ 0], t_integer);
...
{   ref *pdr = osp;  /* font dict */
    gs_font *pfont;
    int code = font_param(imemory, osp, &pfont);
    gs_font_base *pbfont;
    int BBox[4];
    i_plugin_holder *h = i_plugin_get_list(i_ctx_p);
    char *xlatmap = NULL;
    ...
    *success = false;
	...
	if (code < 0) {
        ...
        if ((code = name_ref(imemory, (const byte *)I->ig.d->subtype, 
			     strlen(I->ig.d->subtype), &FAPI_ID, false)) < 0)
	    return code;
