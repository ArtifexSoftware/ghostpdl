...
const byte bin_token_bytes[] =
{bin_token_bytes_values};
...
#define return_refill()\
  { sputback_inline(s, p, rlimit); s_end_inline(s, p, rlimit);\
	...
	case bt_seq_IEEE_msb:
	    num_format = num_msb + num_float_IEEE;
	    goto bseq;
	case bt_seq_IEEE_lsb:
	    num_format = num_lsb + num_float_IEEE;
	    goto bseq;
	case bt_seq_native_msb:
	    num_format = num_msb + num_float_native;
	    goto bseq;
	case bt_seq_native_lsb:
	  ...
	  bseq:pbs->num_format = num_format;
		...
		if (top_size == 0) {	/* Extended header (2-byte array size, */
		    /* 4-byte length) */
		    ulong lsize;
		    ...
		    hsize = 8;
		} else {	/* Normal header (1-byte array size, 2-byte length). */
	    ...
	    if (code == scan_Refill || code < 0) {	/* Clean up array for GC. */
	    ...
	    return code;
	case bt_int32_msb:
	    num_format = num_msb + num_int32;
	    goto num;
	case bt_int32_lsb:
	    num_format = num_lsb + num_int32;
	    goto num;
	case bt_int16_msb:
	    num_format = num_msb + num_int16;
	    goto num;
	case bt_int16_lsb:
	    num_format = num_lsb + num_int16;
	    goto num;
	case bt_int8:
	...
	case bt_num_array:
	    ...
	    if (code == scan_Refill || code < 0) {	/* Make sure the array is clean for the GC. */
...
scan_bin_string_continue(stream * s, ref * pref, scanner_state * pstate)
{
    ...
    if (rcnt == wanted) {	/* Finished collecting the string. */
  ...
  if ( must_swap_bytes ) do_swap_size(), do_swap_value()
	switch (ob.tx & 0x7f) {
		...
		sattrs = (ob.tx < 128 ? a_all : a_all + a_executable);
	      str:if (ob.size.w == 0) {/* For zero-length strings, the offset */
		...
		if (ob.value.w < max_array_index * sizeof_bin_seq_obj ||
		    ...
		    return_error(e_syntaxerror);
		if (ob.value.w < min_string_index) {	/* We have to (re)allocate the strings. */
