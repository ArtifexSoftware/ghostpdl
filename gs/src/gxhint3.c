...
#define pnext_curve ((curve_segment *)pnext)
    ...
    if (pcis->init_done < 0)
	return;
    if (pseg == 0) {		/* Start at the beginning of the subpath. */
	if (psub == 0)
	    return;
	...
	switch (pnext->type) {
		...
		if (line_is_null(pnext->pt, pcis->unmoved_end)) {	/* This is a null line, just move it. */
		    hints_next = hints;
    ...
    if (closing) {		/* Handle the end of the subpath wrapping around to the start. */
