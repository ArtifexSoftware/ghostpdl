...
{
    {"gscmyk", stc_gscmyk, DeviceCMYK | STC_BYTE | STC_DIRECT, 0,
     {0.0, 1.0}},
    {"hscmyk", stc_hscmyk,
...
/* ESC-R color codes, used only here */
    static const byte stc_colors[] =
    {0x02, 0x01, 0x04, 0x00};	/* CMYK */
...
stc_freedata(stc_t * stc)
{
	...
	if (stc->code[i] != NULL) {
	    ...
	    if (i == j)
		gs_free(stc->code[i], 1 << stc->bits, sizeof(gx_color_value),
			"stcolor/code");
	}
	if (stc->vals[i] != NULL) {
	    ...
	    if (i == j)
		gs_free(stc->vals[i], 1 << stc->bits, sd->stc.alg_item,
			"stcolor/transfer");
	}
    }
...
#  undef stc_write_int
    ...
    if (sd->stc.am != NULL) {
	if (nc == 1)
	    set_param_array(pfa, sd->stc.am, 3)
		else
	    if (nc == 3)
		set_param_array(pfa, sd->stc.am, 9)
		    else
		set_param_array(pfa, sd->stc.am, 16)
 ...
 */
	...
	if (code < 0) {		/* free newly allocated arrays */
		...
		if ((sd->stc.extc[i] != NULL) &&
		    ...
		    if (i == j)
			gs_free(sd->stc.extc[i], sd->stc.sizc[i], sizeof(float),
				"stc_put_params");
		}
		if ((sd->stc.extv[i] != NULL) &&
		    ...
		    if (i == j)
			gs_free(sd->stc.extv[i], sd->stc.sizv[i], sizeof(float),
				"stc_put_params");
		}
	    }			/* components */
 ...
 */
	...
	for (i = 0; i < 4; ++i) {
		...
		if (i == j)
		    gs_free(oldstc.extc[i], oldstc.sizc[i], sizeof(float),
			    "stc_put_params");
	    }
		...
		if (i == j)
		    gs_free(oldstc.extv[i], oldstc.sizv[i], sizeof(float),
			    "stc_put_params");
	    }
	}
