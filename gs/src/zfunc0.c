		    ...
		    int depth, gs_function_t ** ppfn, gs_memory_t *mem)
{
    ...
    *(gs_function_params_t *) & params = *mnDR;
	...
	case t_file: {
	    ...
	    check_read_known_file_else(imemory, s, pDataSource, return_error,
    ...
    if ((code = dict_int_param(imemory, op, "Order", 1, 3, 1, &params.Order)) < 0 ||
	(code = dict_int_param(imemory, op, "BitsPerSample", 1, 32, 0,
			       &params.BitsPerSample)) < 0 ||
	((code = fn_build_float_array(op, "Encode", false, true, &params.Encode, mem)) != 
	 2 * params.m && (code != 0 || params.Encode != 0)) ||
	((code = fn_build_float_array(op, "Decode", false, true, &params.Decode, mem)) != 
	 2 * params.n && (code != 0 || params.Decode != 0))
	) {
	goto fail;
    } {
	int *ptr = (int *)
	...
	params.Size = ptr;
	code = dict_ints_param(imemory, op, "Size", params.m, ptr);
	if (code != params.m)
	    goto fail;
    }
