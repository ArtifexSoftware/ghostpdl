...
gx_san__release(gx_device_spot_analyzer **ppadev)
{
    ...
    if (padev == NULL) {
	eprintf(padev->memory, "Extra call to gx_san__release.");
	return;
    }
    if(--padev->lock < 0) {
	eprintf(padev->memory, "Wrong lock to gx_san__release.");
	return;
    }
    if (padev->lock == 0) {
	*ppadev = NULL;
	rc_decrement(padev->memory, padev, "gx_san__release");
    }
}
    ...
    const segment *l, const segment *r, int dir_l, int dir_r)
{
    ...
    if (last == NULL)
	return_error(padev->memory, gs_error_VMerror);
    last->ybot = ybot;
    last->ytop = ytop;
    last->xlbot = xlbot;
    last->xrbot = xrbot;
