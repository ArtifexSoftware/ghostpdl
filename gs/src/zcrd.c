...
/* - .currentcrd <dict> */
private int
zcurrentcrd(register os_ptr op)
{
    stack_param_list list;
    int code;

    stack_param_list_write(&list, &o_stack, NULL);
    code = param_write_CIEABC_render1((gs_param_list *) & list, "CRD",
				      gs_currentcolorrendering(igs),
				      imemory);
    if (code < 0)
	return code;
...
zsetcolorrendering(register os_ptr op)
{
    ...
    check_dict_read(*op);
    rc_alloc_struct_0(pcie, gs_cie_render, &st_cie_render1, mem,
...
cie_cache_render_finish(os_ptr op)
{
    ...
    if (pcie->RenderTable.lookup.table != 0 && !pcie->caches.RenderTableT_is_identity) {	/* Convert the RenderTableT cache from floats to fracs. */
	int j;
    ...
    pcie->EncodeLMN = EncodeLMN_from_cache;
    pcie->EncodeABC = EncodeABC_from_cache;
    pop(1);
    return 0;
}
