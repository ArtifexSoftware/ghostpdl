...
/* ------ Reading/rendering ------ */
typedef enum {
    playback_action_render, playback_action_setup
...
#define set_cb_end(p)\
  ...
  if ( cb_limit > cb_end ) cb_limit = cb_end
	if (cbp >= cb_limit) {
	    if (end_status < 0) {	/* End of file or error. */
		if (cbp == cb_end) {
...
/****** DOESN'T HANDLE #PLANES YET *****/
				...
				if (cb_end - cbp >= nbytes) {
				    ...
				    if (nbytes > cb_end - cbuf) {	/* Allocate a separate buffer. */
					rdata = data_on_heap =
