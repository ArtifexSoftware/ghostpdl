			 ...
			 int data_x, uint iw, int h, gx_device * dev)
{
    ...
    if (h != 0) {		/* Convert the unpacked data to concrete values in */
	/* the source buffer. */
	uint row_size = pss->WidthIn * c * pss->sizeofPixelIn;
	const byte *bdata = buffer + data_x * c * pss->sizeofPixelIn;

	if (pss->sizeofPixelIn == 1) {	/* Easy case: 8-bit device color values. */
	    r.ptr = bdata - 1;
	} else {		/* Messy case: concretize each sample. */
	    int bps = penum->bps;
	    int dc = penum->spp;
	...
	for (;;) {
		...
		for (x = xo; x < xo + width; x++, psrc += c) {
		    ...
		    if (color_is_pure(&devc)) {		/* Just pack colors into a scan line. */
