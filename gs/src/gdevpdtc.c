...
process_composite_text(gs_text_enum_t *pte, void *vbuf, uint bsize)
{
	...
	for (;;) {
		...
		if (chr != (byte)chr)	/* probably can't happen */
		    return_error(pte->memory, gs_error_rangecheck);
		if (buf_index >= bsize)
		    return_error(pte->memory, gs_error_unregistered); /* Must not happen. */
		...
		const gs_cmap_t *pcmap, int font_index_only)
{
	 ...
	 */
	is_identity = gs_cmap_is_identity(pdev->memory, pcmap, font_index_only);
    }
...
scan_cmap_text(pdf_text_enum_t *pte)
{
	...
	if (code < 0)
		...
		if (!(pdsubf->used[cid >> 3] & (0x80 >> (cid & 7)))) {
		    ...
		    buf[l] = 0;
		    eprintf2(pdev->memory, "Missing glyph CID=%d in the font %s . The output PDF may fail with some viewers.\n", cid, buf);
	...
	if (break_index > index) {
	    ...
	    if (code < 0)
		return code;
	    code = gs_matrix_invert(pte->memory, &m0, &m1);
	    if (code < 0)
		return code;
