...
struct gs_color_space_type_s {
	 ...
	 * underlying space, this value is -1.
	 */
	...
	/*
	 * Define the size of the smallest gs_color_space_ structure that
	 * can hold this space (or, equivalently, the actual size of the
	 * color space structure).  This allows us to copy color spaces into
	 * and out of the base_space and alt_space members of parameterized
	 * types.
	 */
...
#define cs_declare_memory_procs(scope, adjust, enum_p, reloc_p)\
...
#define cs_declare_procs(scope, concretize, install, adjust, enum_p, reloc_p)\
  ...
  cs_declare_memory_procs(scope, adjust, enum_p, reloc_p)
