 ...
 */
PRIVATE const int reschedule_interval = 100;
...
zcontext_init(i_ctx_t *i_ctx_p)
{
    ...
    if (i_ctx_p) {
	i_ctx_p->interp_reschedule_proc = ctx_initialize;
	i_ctx_p->interp_time_slice_proc = ctx_initialize;
	i_ctx_p->interp_time_slice_ticks = 0;
    }
    return 0;
}
/*
...
ctx_initialize(i_ctx_t **pi_ctx_p)
{
    ...
    *pi_ctx_p = &psched->current->state;
    (*pi_ctx_p)->interp_reschedule_proc = ctx_reschedule;
    (*pi_ctx_p)->interp_time_slice_proc = ctx_time_slice;
    (*pi_ctx_p)->interp_time_slice_ticks = reschedule_interval;
    return 0;
}
...
PRIVATE int do_fork(P6(i_ctx_t *i_ctx_p, os_ptr op, const ref * pstdin,
		       const ref * pstdout, uint mcount, bool local));
PRIVATE int values_older_than(P4(const ref_stack_t * pstack, uint first, uint last,
				 int max_space));
VIRTUAL int fork_done(P1(i_ctx_t *));
VIRTUAL int fork_done_with_error(P1(i_ctx_t *));
VIRTUAL int finish_join(P1(i_ctx_t *));
VIRTUAL int reschedule_now(P1(i_ctx_t *));
	...
	uint mcount, bool local)
{
	 ...
	 */
	code = dict_alloc(lmem, userdict_size, &new_userdict, &i_ctx_p->dict_stack.dict_defaults);
	if (code < 0) {
	    context_destroy(pctx);
	    /****** FREE lmem ******/
	    return code;
	       ...
	       const gs_context_state_t *i_ctx_p, bool copy_state)
{
	...
	code = context_state_alloc(&pctx_st, systemdict, &i_ctx_p->dict_stack.dict_defaults, dmem);
	if (code < 0) {
