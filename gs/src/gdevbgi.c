...
bgi_open(gx_device * dev)
{
    ...
    if (bgi_path == NULL)
	bgi_path = BGI_LIB;
    if (bgi_user != NULL) {	/* A user-supplied driver is specified as "mode.dname", */
	...
	driver = SUPER_VGA;
    } else {			/* not user-defined driver *//* We include the CGA driver in the executable, */
	...
	if (driver == EGA64) {	/* Select 16 color video mode if video card is EGA with 64 Kb of memory */
	    mode = EGA64LO;
	}
	...
	if (driver == CGA) {	/* The actual hardware might be an AT&T 6300. */
	    /* Try initializing it that way. */
	    int save_mode = mode;

	    driver = ATT400, mode = ATT400HI;
	    initgraph(&driver, &mode, bgi_path);
	    if (graphresult() != grOk) {	/* Nope, it was a real CGA. */
		closegraph();
    ...
    if (dev->y_pixels_per_inch == 1) {	/* Get the aspect ratio from the driver. */
	int arx, ary;
	       ...
	       int x, int y, int w, int h)
{
    ...
    } else {			/* monochrome device: one bit per pixel *//* bitmap is the same as bgi_copy_mono: one bit per pixel */
		 ...
		 gs_point * pwidth, gs_int_rect * pbbox)
{
    ...
    pwidth->y = 0;
    if (pwidth->x == pbbox->q.x && pbbox->p.x == 0) {	/* This is a badly designed font with no inter-character */
	/* spacing.  Add 1 pixel. */
	pwidth->x++;
    }
		...
		int xo, int yo, gx_color_index color, int required)
{
    ...
    if (target->dname == gs_bgi_device.dname) {		/* Write directly to a BGI device. */
	ci.str[0] = (char)xg;
	ci.str[1] = 0;
	setcolor((int)color);
