		...
		gx_color_index color0, gx_color_index color1)
{
  ...
  copy:{
	...
	compress = (uint) code;
	if (code < 0) {		/* The bitmap was too large; split up the transfer. */
	    if (height > 1) {	/* Split the transfer by reducing the height.
		     ...
		     int phase_x, int phase_y, gs_logical_operation_t lop)
{
	...
	if (tcolors == 0 || tcolors[0] != tcolors[1]) {
	    ...
	    if (!cls_has_tile_id(cdev, pcls, tiles->id, offset_temp)) {		/* Change tile.  If there is no id, generate one. */
