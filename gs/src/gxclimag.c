		...
		gs_logical_operation_t lop, const gx_clip_path * pcpath)
{
	...
	if (!cls_has_tile_id(cdev, pcls, id, offset_temp)) {
	    ...
	    if (code < 0) {	/* Something went wrong; just copy the bits. */
		goto copy;
	    }
	} {
	    gx_cmd_rect rect;
...
private dev_proc_end_image(clist_image_end_image);
...
/* This is really an old-style image_data procedure. */
private int
     ...
     gx_image_enum_common_t * info, const gx_image_plane_t * planes, int yh)
{
    ...
    uint raster = planes[0].raster;
    const byte *plane_data[5];
    int i;

    for (i = 0; i < pie->num_planes; ++i)
	plane_data[i] = planes[i].data;
    if (pie->default_info)
	return gx_default_image_data(dev, pie->default_info, plane_data,
				     data_x, raster, yh);
    sbox.p.x = pie->rect.p.x;
    sbox.p.y = y0 = pie->y;
    sbox.q.x = pie->rect.q.x;
...
#define by1 ibox.q.y
	...
	for (iy = by0, ih = by1 - by0; ih > 0;
	    ...
	    TRY_RECT
		code = cmd_image_data(cdev, pcls, plane_data,
				      pie->num_planes,
