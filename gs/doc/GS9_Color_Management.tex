...
\begin{center}{\huge \bf Ghostscript 9.06 Color Management\\} \vspace{0.5in} {\Large Michael J.
Vrhel, Ph.D.\\} {\Large Artifex Software\\} {\Large 7 Mt. Lassen Drive, A-134\\} {\Large San Rafael, CA 94903, USA\\}
{\Large www.artifex.com\\}
\end{center}
\vspace*{0.5in}
\begin{abstract}
This document provides information about the color architecture in Ghostscript 9.06. The document is suitable for users who wish to
...
Similarly, it is possible to turn off or on black point compensation for the color managed objects in the document.  Black point compensation is a mapping performed near the black point that ensures that the luminance black in a source color space is mapped to the luminance black in a destination color space with adjustments to ensure a smooth transition for near black colors.  The mapping is similar to the mapping performed at the white point between devices. With black point compensation enabled, potential loss of detail in the shadows is reduced.   By default, Ghostscript has black point compensation  enabled.  However, note that the PDF 2.0 specification adds a black point compensation member to the extended graphic state.  As such, it is possible that the document could turn off black point compensation.   If this is not desired, it is possible to force black point compensation to a particular state using two parameter settings which are\\

\textcolor{red}{-dOverrideBP = true/false}\\

\noindent which, when set to true overrides the black point compensation contained in the source document with the black point compensation that has been specified by\\
...
The following commands, enable one to specify unique {\bf output} ICC profiles, rendering intents and black point compensation for text, graphic and image objects.  As shown in Figure \ref{fig:ICC_ARCH}, these profiles are stored in the device structure.  Specifically, the command options are:\\
...
\noindent Sets the black point compensation setting that should be used with the profile specified above by -sGraphicICCProfile.  It is also necessary to set -dOverrideBP=true.\\
...
\noindent Sets the black point compensation setting that should be used with the profile specified above by -sImageICCProfile.  It is also necessary to set -dOverrideBP=true.\\
...
\noindent Sets the black point compensation setting that should be used with the profile specified above by -sTextICCProfile.  It is also necessary to set -dOverrideBP=true.\\
...
\begin{tabular}{llll}
Graphic\_CMYK & cmyk\_src\_renderintent.icc	& 0 & 1 \\
Image\_CMYK	& cmyk\_src\_renderintent.icc	& 1 & 1 \\
Text\_CMYK	& cmyk\_src\_renderintent.icc	& 2 & 1 \\
\end{tabular}\\
