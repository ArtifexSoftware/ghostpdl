...
pdfi_t1_glyph_data(gs_font_type1 * pfont, gs_glyph glyph, gs_glyph_data_t *pgd)
{
    ...
    if (glyph >= gs_c_min_std_encoding_glyph) {
        ...
        if (code >= 0) {
            code = pdfi_make_name(pdffont1->ctx, (byte *)str.data, str.size, (pdf_obj **)&glyphname);
        }
    }
    else {
...
static int
pdfi_t1_enumerate_glyph(gs_font *pfont, int *pindex, gs_glyph_space_t glyph_space, gs_glyph *pglyph)
{
    int code;
...
static int
pdfi_t1_glyph_outline(gs_font *pfont, int WMode, gs_glyph glyph, const gs_matrix *pmat, gx_path *ppath, double sbw[4])
{
    gs_glyph_data_t gd;
...
push_pfb_filter(gs_memory_t *mem, byte *buf, byte *bufend)
{
    ...
    st = gs_alloc_struct(mem, stream_SFD_state, s_PFBD_template.stype, "push_pfb_filter(st)");
...
static int pdfi_t1_decode_pfb(pdf_context *ctx, byte *inbuf, int inlen, byte **outbuf, int *outlen)
{
    stream *strm;
    int c, code = 0;
    int decodelen = 0;
...
static int pdfi_open_t1_font_file(pdf_context *ctx, char *fontfile, byte **buf, int64_t *buflen)
{
    int code = 0;
...
int pdfi_read_type1_font(pdf_context *ctx, pdf_dict *font_dict, pdf_dict *stream_dict, pdf_dict *page_dict, gs_font **ppfont)
{
    int code;
    byte *fbuf = NULL;
    int64_t fbuflen = 0;
    ...
    if (fontfile != NULL) {
        ...
        if (code < 0) {
            ...
            code = pdfi_make_name(ctx, (byte *)"Helvetica", 9, &defname);
...
int pdfi_free_font_type1(pdf_obj *font)
{
    ...
    pdf_context *ctx = (pdf_context *)t1f->ctx;
    int i;

    gs_free_object(ctx->memory, t1f->pfont, "Free Type 1 gs_font");
    ...
    if (t1f->fake_glyph_names != NULL) {
        for (i = 0; i < t1f->LastChar; i++) {
            if (t1f->fake_glyph_names[i].data != NULL)
                gs_free_object(ctx->memory, t1f->fake_glyph_names[i].data, "Type 1 fake_glyph_name");
        }
        gs_free_object(ctx->memory, t1f->fake_glyph_names, "Type 1 fake_glyph_names");
    }
    if (t1f->NumSubrs > 0 && t1f->Subrs != NULL) {
        for (i = 0; i < t1f->NumSubrs; i++) {
            gs_free_object(ctx->memory, t1f->Subrs[i].data, "Type 1 Subr");
        }
        gs_free_object(ctx->memory, t1f->Subrs, "Type 1 Subrs");
    }
    gs_free_object(ctx->memory, t1f->Widths, "Free Type 1 fontWidths");
    gs_free_object(ctx->memory, t1f, "Free Type 1 font");
    return 0;
}
