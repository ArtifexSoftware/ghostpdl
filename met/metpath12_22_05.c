...
private void 
split(char *b, char **args, char delim)
{
    while (*b != NULL) {
        while (*b == delim)
            *b++ = NULL;
        *args++ = b;
        while((*b != NULL) && (*b != delim))
            b++;
    }
    *args = NULL;
}
...
ArcSegment_action(void *data, met_state_t *ms)
{
    ...
    */
    ...
    {
        float rtx[4]; /* not used */
        if ((code = gs_arcto(pgs, start.x, start.y, end.x, end.y, size.x, rtx)) < 0)
            return code;
    }
